...
The module root links the browser and carries its page API - `New`, `Open`,
`Browser`, `Page`, `BrowserOptions`, `ScreenshotOptions`, `BrowserRequest`,
`BrowserResponse`, `Block`, `Fulfill` - so the import above is all a program
needs to fetch, render, read and drive a page. `Open` takes what `Browse` takes
and answers with the page object instead of the rendered bytes. The browser
names that would be ambiguous here keep their own spelling: `Get`, `Options`,
`Request` and `Response` are the HTTP verb and messages in this namespace, so
the browser package's shortcuts of those names come from importing it. A
program that imports `requests` alone leaves the browser out and is told so if
it asks for one.
