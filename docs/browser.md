...
module root carries the page API too - as `Open`, `New`, `BrowserOptions`,
`Page` and `ScreenshotOptions` - so a program already importing the facade
needs no second import to drive a page; `Open` is this package's `Get` under
the name the facade can carry, taking a `Request` and answering with the page.
Its `Get`, `Options`, `Request` and `Response` keep this package's spelling,
because those names are the HTTP verb and messages there.
